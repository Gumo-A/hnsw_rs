...
pub fn parse_args_eval() -> Result<(usize, usize), &'static str> {
    let args: Vec<String> = env::args().collect();
    ...
    let lim = args[1].parse::<usize>().expect("Could not parse limit");
    let m = args[2].parse::<usize>().expect("Could not parse M");
    Ok((lim, m))
}
